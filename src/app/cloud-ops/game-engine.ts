...
export const scenarios: Record<ScenarioId, Scenario> = {
  'connection-storm': { id: 'connection-storm', codename: 'POOL PRESSURE', pager: 'CHECKOUT READINESS COLLAPSE · USER ERRORS LOW', clue: 'Replica count changed minutes before readiness began failing.', rootCause: 'Replica count multiplied a fixed per-pod database pool beyond the server connection ceiling.', hint: 'Compare replicas × pool size with the database maximum.' },
  'poison-release': { id: 'poison-release', codename: 'LAST GOOD IMAGE', pager: 'CHECKOUT RESTART LOOP · RSS CLIMBING', clue: 'Only pods on the newest image exhibit the failure pattern.', rootCause: 'The new image has a memory regression and repeatedly reaches its container limit.', hint: 'Inspect restart reason and image history before changing capacity.' },
  'dns-fracture': { id: 'dns-fracture', codename: 'NAME WITHOUT A ROUTE', pager: 'UPSTREAM TIMEOUTS · APPLICATION HEALTH NORMAL', clue: 'Requests fail before they ever reach the application service.', rootCause: 'Internal DNS resolution is intermittently failing, so service names cannot be resolved reliably.', hint: 'Trace a request and inspect the DNS hop instead of scaling the app.' },
  'certificate-expiry': { id: 'certificate-expiry', codename: 'EDGE OF TRUST', pager: 'PUBLIC TRAFFIC FAILING · INTERNAL HEALTH GREEN', clue: 'Cluster-to-cluster traffic is healthy while external TLS sessions fail.', rootCause: 'The ingress certificate expired at the production boundary.', hint: 'Inspect the edge certificate rather than the pods.' },
  'az-failure': { id: 'az-failure', codename: 'DARK ZONE', pager: 'AZ-B PACKET LOSS · CAPACITY DEGRADING', clue: 'One zone loses network quality while the surviving zone remains healthy but hot.', rootCause: 'Availability-zone degradation is shedding usable capacity and concentrating load elsewhere.', hint: 'Drain the unhealthy zone and protect capacity before changing the release.' },
};

const order: ScenarioId[] = ['connection-storm', 'poison-release', 'dns-fracture', 'certificate-expiry', 'az-failure'];

export function scenarioForSeed(seed: number): ScenarioId { return order[Math.abs(seed) % order.length]; }

export function createInitialState(scenario: ScenarioId): GameState {
  const telemetry: Telemetry = { latency: 140, errors: 0.4, cpu: 48, ready: 3, replicas: 3, dbConnections: 60, dbMax: 90, kafkaLag: 8 };
  ...
  return { scenario, phase: 'briefing', telemetry, selected: null, elapsed: 0, mitigations: [], discoveries: [], harmfulActions: 0, log: ['02:17:00  Pager acknowledged.', '02:17:04  Production model attached.'] };
}

function addUnique(items: string[], item: string) { return items.includes(item) ? items : [...items, item]; }
function markDiscovery(state: GameState, item: string): GameState { return { ...state, phase: state.phase === 'briefing' ? 'investigating' : state.phase, discoveries: addUnique(state.discoveries, item) }; }

export function selectNode(state: GameState, node: NodeId): GameState {
  let next: GameState = { ...state, selected: node, elapsed: state.elapsed + 12, log: [...state.log, `02:${String(17 + Math.floor((state.elapsed + 12) / 60)).padStart(2, '0')}:${String((state.elapsed + 12) % 60).padStart(2, '0')}  Inspected ${node}.`] };
...
function resolved(state: GameState, action: string, telemetry: Partial<Telemetry>): GameState {
  return { ...state, phase: 'mitigated', elapsed: state.elapsed + 48, telemetry: { ...state.telemetry, ...telemetry }, mitigations: addUnique(state.mitigations, action), log: [...state.log, `MITIGATION ACCEPTED · ${action}`] };
}
function harmful(state: GameState, action: string, telemetry: Partial<Telemetry>): GameState {
  ...
  return { ...state, phase: harmfulActions >= 3 ? 'failed' : 'investigating', elapsed: state.elapsed + 34, harmfulActions, telemetry: { ...state.telemetry, ...telemetry }, log: [...state.log, `CHANGE WORSENED SYSTEM · ${action}`] };
}
...
export function runCommand(state: GameState, raw: string): TerminalResult {
  ...
  let next: GameState = { ...state, elapsed: state.elapsed + 8, phase: state.phase === 'briefing' ? 'investigating' : state.phase, log: [...state.log, prompt] };
  ...
  if (lower === 'kubectl describe ingress') { if (state.scenario === 'certificate-expiry') next = markDiscovery(next, 'tls-expiry'); return { state: next, lines: [prompt, state.scenario === 'certificate-expiry' ? 'TLS secret checkout-prod · certificate EXPIRED 02:00 UTC' : 'TLS secret checkout-prod · certificate valid'] }; }
  if (lower.startsWith('dig ')) { if (state.scenario === 'dns-fracture') next = markDiscovery(next, 'dns-loss'); return { state: next, lines: [prompt, state.scenario === 'dns-fracture' ? ';; communications error to 10.96.0.10#53: timed out' : 'checkout.svc.cluster.local. 30 IN A 10.42.3.18'] }; }
